...
def write_milp(
    ...
    model_str = check_redundancy_and_write_indicator_constraint(model_str, constr_cache, indicator, False, *expr_false, **kwargs)
    if not (low == sympy.oo or low == -sympy.oo):
        model_str = check_redundancy_and_write_indicator_constraint(model_str, constr_cache, indicator, False, var_node, low, **kwargs)
    kwargs['dec_id'] = str(dec)
    model_str = check_redundancy_and_write_indicator_constraint(model_str, constr_cache, indicator, True, *expr_true, **kwargs)
    if not (high == sympy.oo or high == -sympy.oo):
        model_str = check_redundancy_and_write_indicator_constraint(model_str, constr_cache, indicator, True, var_node, high, **kwargs)
...
def write_milp_from_bilevel(
    ...
    # Recursively write the MILP model
    low, constrCache, model_str = write_milp_from_bilevel(model_str, context, node._low, node_id_to_var_str, constrCache,
                                                          binary_set, continuous_set, binary, data, data_idx, **kwargs)
    high, constrCache, model_str = write_milp_from_bilevel(model_str, context, node._high, node_id_to_var_str, constrCache,
                                                           binary_set, continuous_set, binary, data, data_idx, **kwargs)

    kwargs['data_idx'] = data_idx
    kwargs['dec_id'] = f'n{dec}'
    model_str = check_redundancy_and_write_indicator_constraint(model_str, constrCache, indicator, False, *expr_false, **kwargs)
    model_str = check_redundancy_and_write_indicator_constraint(model_str, constrCache, indicator, False, var_node, low, **kwargs)
    kwargs['dec_id'] = str(dec)
    model_str = check_redundancy_and_write_indicator_constraint(model_str, constrCache, indicator, True, *expr_true, **kwargs)
    model_str = check_redundancy_and_write_indicator_constraint(model_str, constrCache, indicator, True, var_node, high, **kwargs)
