...
from xaddpy.models.spo_plus import SPOPlusClassify
from xaddpy.models.qptl import QPTLClassify
from xaddpy.models.intopt import IntOptClassify
from xaddpy.models.two_stage import TwoStageClassify
...
from xaddpy.experiments.predopt.classification.util import generate_dataset, make_json
...
def run_baselines(alg: str, *args, **kwargs):
    cls = globals()[alg]
    bs = cls(*args, **kwargs)
    init_cost, opt_cost, init_regret = bs.test_model(bs.X_train, bs.y_train, dom='classification')
...
def run(args):
    ...
    linear = True if len(embed_dim) == 0 or args.linear else False
    ...
    dir_path = path.join(path.curdir, 'xaddpy/experiments/predopt/classification')
    ...
    fname_json = make_json(class_dim, feature_dim, args)        # Create the .json file
    ...
    for r in range(num_runs):
        ...
        target_dim = c_train_r.shape[1]  # Number of classes = number of decision variables
        ...
        if algo == 'emspo':
            ...
            emspo_res = {'emspo': test_algorithm(emspo, dataset, 'train', 'classification')}
            res.update(emspo_res)

        else:
            baselines = ['twostage', 'spoplus', 'qptl', 'intopt'] if algo == 'baselines' else [algo]
            bs_dict = {'spoplus': 'SPOPlusClassify', 'twostage': 'TwoStageClassify',
                       'qptl': 'QPTLClassify', 'intopt': 'IntOptClassify'}
            ...
            # Check train loss
            bs_res = {k: test_algorithm(alg_inst_dict[k], dataset=dataset, config='train', domain='classification')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--class_dim', type=int, default=10, help='Number of classes')
    parser.add_argument('--feature_dim', type=int, default=5, help='Feature dimensionality')
    parser.add_argument('--size', type=int, default=200, help='Number of data instances')
    parser.add_argument('--eps_bar', type=float, default=0.5, help='Multiplicative noise half-width')
    ...
    parser.add_argument('--standardize', dest='standardize', default=False, action='store_true',
