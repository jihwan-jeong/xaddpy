...
Code copied from https://github.com/JayMan91/NeurIPSIntopt/blob/main/Interior/remove_redundancy.py
"""
...
def _remove_redundancy(A):
    ...
    while abs(s_min) < tol:
        ...
        i_remove = _get_densest(A_, eligibleRows)
...
def _remove_redundant_rows(A):
    # remove redundant (linearly dependent) rows from equality constraints
    redundancy_warning = ("A does not appear to be of full row rank. To "
    ...
    if A.size > 0:
        try:  # TODO: instead use results of first SVD in _remove_redundancy
            ...
            rank = 0
    if A.size > 0 and rank < A.shape[0]:
        warn(redundancy_warning, OptimizeWarning, stacklevel=3)
        dim_row_nullspace = A.shape[0] - rank
        if dim_row_nullspace <= small_nullspace:
            d_removed, status, message = _remove_redundancy(A)
        if dim_row_nullspace > small_nullspace:
            d_removed, status, message = _remove_redundancy_dense(A)
        if A.shape[0] < rank:
            message = ("Due to numerical issues, redundant equality "
                       "constraints could not be removed automatically. "
                       "Try providing your constraint matrices as sparse "
