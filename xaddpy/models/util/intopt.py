"""
Code directly copied from https://github.com/JayMan91/NeurIPSIntopt/tree/main/Interior
"""
import numpy as np
import scipy as sp
...
from xaddpy.models.util.linalg import _remove_redundancy, _remove_redundancy_sparse, _remove_redundancy_dense
        ...
        while (not solved and attempt_count < 3):
            try:
                ...
                if np.any(np.isnan(p)) or np.any(np.isnan(q)):
                    raise (LinAlgError)
                solved = True
                # logger.log("Success in finding delta ----- solving attempt %d " % (attempt_count))
            except (ValueError, TypeError, LinAlgError) as e:
                # logger.log("Cholesky failed trying other method ----- solving attempt %d error message %s" % (
                # attempt_count, e))
    ...
    class IPOfunc_cls(Function):
        ...
        def forward(ctx, c):
            ...
            try:
                ...
                # logger.info("Shape of A_eq {} A_ineq {} at the beginning".format(A_.shape,G_.shape))

            except ValueError:

                # c contains  inf, nan, or None
                logger.info("####### Alert #########")
                logger.info("matrix contains inf, nan, or None")
                logger.info("C containes nan ? {} Infinity? {} ".format(np.isnan(c_).any(),
                                                                         np.isinf(c_).any()))
                logger.info("smoothing {} thr {}".format(smoothing, thr))
                logger.info("####### #########")
                raise (LinAlgError)
                # c_ = np.nan_to_num(c_)
            ...
            if presolve:
                ...
                # logger.info("After pre solve shape of c {} of A{} b{} h{} G{}".format(c_.shape,
                #     A_.shape,b_.shape,h_.shape,G_.shape))
            if len(undo) > 0:
            ...
            if not complete:
                # logger.info("Shape of A_eq {} A_ineq {} before getabc".format(A_.shape,G_.shape))
                A_, b_, c_, c0, x0 = _get_Abc(c_, G_, h_, A_, b_, bounds_, x0, undo, c0)
                # logger.info("Shape of A after getabc {}".format(A_.shape))
                if c0 != 0:
                    logger.info("c0 is not zero! {} ".format(c0))
                ...
                while go:
                    ...
                    # logger.info("d_x %s  ,d_tau  %s, d_kappa %s"%(d_x,d_tau, d_kappa))
                    # print(solved)
                    # if not solved:
                    #    logger.info(" G %s, h %s, c %s" %(G,h,c))
                    ...
                    if not solved:
                        ...
                        logger.info("not able to find solutions")
                        problem_solved = False
                        break
                    # print(np.sum(x_hat))
                    # print(c.dot(x_hat))
                ...
                # if kappa > 1e-3:
                #     logger.info("kappa >0 ; c.x {}, b.y {}".format(c_.dot(x),
                #                                                     b_.dot(y)))

                # logger.info("calculated mu {:.2f}".format((x.dot(t) + np.dot(tau, kappa)) / (len(x) + 1)))

                x_hat = x / tau
                t_hat = t / tau
                y_hat = y / tau
                # logger.info("calculated mu {:.2f}".format((x_hat.dot(t_hat) ) / (len(x) + 1)))
        ...
        def backward(ctx, del_x):
            ...
            # logger.info("shape of x {} y {} t {} c {} A {} b {}".format(x.shape,
            ...
            # logger.info("Value of mu: {:f} and n {}".format(mu.item(), n))
            ...
            try:
                ...
                # logger.info("rank of M  {}and shape of M {}".format(rank, M_numpy.shape))
            ...
            # logger.info("shape:: p  {} q  {} u {} v {}".format(p.shape, q.shape,
            #     u.shape, v.shape))
            ...
            if any(torch.isnan(c_grad).tolist()):
                logger.info("nan in bkwd pass ; del_x contains NaN?- {}".format(any(torch.isnan(del_x).tolist())))
            if any(torch.isinf(c_grad).tolist()):
                logger.info("Inf in bkwd pass ; del_x contains Inf?- {}".format(any(torch.isinf(del_x).tolist())))
