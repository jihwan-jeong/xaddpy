...
def create_prob_inst(args, seed):
    ...
    if args.cfg is not None:
        ...
        if args.skip_generation:
            fname_json = f'./examples/dblp/vincente/prob_instances/dblp_inst_{k11}_{k12}_{k13}_{k1}_{k2}_opt_{args.opt_var}_seed{seed}.json'
        ...
        if args.skip_generation:
            ...
            if not check:
                logger.info(f"File {fname_json} does not exist! Create...")
        if (args.skip_generation and not check) or not args.skip_generation:
            fname_json = dblp_util.create_prob_json(seed, density=args.density, nx=args.nx, ny=args.ny)
    return fname_json
...
if __name__ == '__main__':
    ...
    parser.add_argument('--domain', dest='domain', default='vincente', type=str)
